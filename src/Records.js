...
export default function Records() {
  ...
  const [records, setRecords] = useState({ male: [], female: [] });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const recordsSnapshot = await getDocs(collection(db, "competitions"));
        ...
        setRecords({ male, female });
      } catch (err) {
        console.error("Errore nel caricamento dei record:", err);
        setError("Errore nel caricamento dei record");
      } finally {
        setLoading(false);
      }
    };
  ...
  const applyRecordFilters = (recordsList) =>
  ...
  const renderRecordsTable = (recordsList, gender) => {
    const filteredRecords = sortRecordsByOrder(applyRecordFilters(recordsList));

    if (filteredRecords.length === 0) {
      return (
        <div className="records-table-section" key={gender}>
  ...
  const recordsForType = [...records.male, ...records.female].filter(
  ...
  const getVisibleSections = () => {
    ...
    if (recordGender === "all" || recordGender === "male") {
      sections.push({
        ...
        rows: sortRecordsByOrder(applyRecordFilters(records.male)),
      });
    }
    if (recordGender === "all" || recordGender === "female") {
      sections.push({
        ...
        rows: sortRecordsByOrder(applyRecordFilters(records.female)),
      });
    }
    return sections;
  };
          ...
          {recordType === "" ? (
            ...
            <>
              {(recordGender === "all" || recordGender === "male") &&
                renderRecordsTable(records.male, "male")}
              {(recordGender === "all" || recordGender === "female") &&
                renderRecordsTable(records.female, "female")}
            </>
          )}
        </div>
      </section>
